...
“[Collection Editing with MVC](https://www.abstractmethod.co.uk/blog/2017/12/collection-editing-with-mvc/)” details these conventions.
...
The `.one` [📖 [docs](https://api.jquery.com/one/#one-events-data-handler)] jQuery function is used for the `.cmd.remove` click event. This is done because this event should fire only once per `fieldset`.
...
Without this explicit passing, a `POST` to an endpoint adorned with `ValidateAntiForgeryToken` [📖 [docs](https://docs.microsoft.com/en-us/aspnet/core/security/anti-request-forgery?view=aspnetcore-6.0)] will throw a Bad Request error.

Both `__RequestVerificationToken` and `maxId` are passed as a plain-old JavaScript object (which kind of looks like JSON), `POST`ed as `application/x-www-form-urlencoded`. ASP.NET Core auto-magically maps `maxId` to the argument of the same name on `TodosController.AddRow`. This convention is super convenient and should eliminate the need to have controllers that read raw JSON from a steam or have an argument of type `Newtonsoft.Json.Linq.JObject` [📖 [docs](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_Linq_JObject.htm)].
...
I am a bit embarrassed to admit that not only do I _not_ use the regular expression flavor of `replace` very often but I also had no idea there was a “replacer” function feature [📖 [docs](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace)]:
